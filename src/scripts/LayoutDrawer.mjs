...
export default class LayoutDrawer {
	...
	static async addSpeakingActor(chatDisplayData) {
		...
		let previousLength = QueueHandler.onscreen.length;
		...
		} else if (QueueHandler.leftScreen.M1 === undefined) {
			columnIdentifier = "L";
			row = 0;
			column = 0;
			QueueHandler.leftScreen.M1 = chatDisplayData.name;
		...
		let scrollFn = setInterval(function () {
		...
		let timeout = LayoutDrawer._wordCount(chatDisplayData.text) * (1000 * LayoutDrawer.secondsPerWord);
